...
if [ $BRANCH == 'master' ]; then
  BRANCH_NAME=latest
  CURA_VERSION_MAJOR="0"
  CURA_VERSION_MINOR="0"
  CURA_VERSION_PATCH="0"
else
  SPLIT=$1
  arrSPLIT=(${SPLIT//./ })
  CURA_VERSION_MAJOR=${arrSPLIT[0]} 
  CURA_VERSION_MINOR=${arrSPLIT[1]}
  CURA_VERSION_PATCH="1"

  CURA_BRANCH_OR_TAG=$BRANCH
  URANIUM_BRANCH_OR_TAG=$BRANCH
  CURAENGINE_BRANCH_OR_TAG=$BRANCH
  CURABINARYDATA_BRANCH_OR_TAG=$BRANCH
  FDMMATERIALS_BRANCH_OR_TAG=$BRANCH
...
docker run \
  ...
  --env FDMMATERIALS_BRANCH_OR_TAG="${FDMMATERIALS_BRANCH_OR_TAG}" \
